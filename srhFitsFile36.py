...
class SrhFitsFile():
    ...
    def calculatePhaseCalibration(self, baselinesNumber = 1, lcp = True, rcp = True):
       for freq in range(self.freqListLength):
           self.solarPhase(freq)
           self.updateAntennaPhase(freq, baselinesNumber, lcp = lcp, rcp = rcp)
           
    def calculateAmpCalibration(self, baselinesNumber = 3):
       for freq in range(self.freqListLength):
           self.calculateAmplitude_linear(freq, baselinesNumber)

    def updateAntennaPhase(self, freqChannel, baselinesNumber = 3, lcp = True, rcp = True):
...
#        self.ewAntPhaLcp[freqChannel] = NP.angle(self.ew_gains_lcp)
        ...
        ls_res = least_squares(self.allGainsFunc_constrained, x_ini, args = (redundantVisAll, antNumberEW, antNumberN, baselinesNumber, freqChannel), max_nfev = 400)
...
#        self.ewAntPhaRcp[freqChannel] = NP.angle(self.ew_gains_rcp)
        ...
        x_size = (baselinesNumber-1)*2 + antNumberEW + antNumberN
        x_ini = NP.concatenate((NP.ones(x_size+1), NP.zeros(x_size)))
        ls_res = least_squares(self.allGainsFunc_constrained, x_ini, args = (redundantVisAll, antNumberEW, antNumberN, baselinesNumber, freqChannel), max_nfev = 500)
